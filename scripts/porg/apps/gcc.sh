...
cd        ${HOME}/.porg/${HOSTNAME}/src/gcc && pwd

## Download
export VERSION="8.5.0"
wget --no-check-certificate https://ftp.gnu.org/gnu/gcc/gcc-${VERSION}/gcc-${VERSION}.tar.gz
wget --no-check-certificate https://ftp.gnu.org/gnu/gcc/gcc-${VERSION}/gcc-${VERSION}.tar.gz.sig
tar -xzvf gcc-*.tar.gz
cd gcc-*/ && pwd
